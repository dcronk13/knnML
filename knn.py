...
def get_nearest_neighbors(example_set, query, k):
    arr = example_set - query
    distList = []
    for row in arr:
        dist = np.linalg.norm(row)
        distList.append(dist)
    idx_of_nearest = np.argsort(distList)
...
def knn_classify_point(examples_X, examples_y, query, k):
    nnIdx = get_nearest_neighbors(examples_X,query,k)
    kLabels = []
    for nn in nnIdx:
        val = examples_y[nn]
        kLabels.append(val[0])
    predicted_label = st.mode(kLabels)
...
def cross_validation(train_X, train_y, num_folds=4, k=1):
    ...
    scores = []
    for foldX,foldY in zip(folds_x,folds_y):
        score = []
        for f,v in zip(folds_x,folds_y):
            if (not np.array_equal(foldX,f)):
                predicted_y = predict(f,v,foldX,k)
                score.append(compute_accuracy(foldY,predicted_y))
        scores.append(np.mean(score))
    avg_val_acc = np.mean(scores)
    varr_val_acc = np.var(scores)
